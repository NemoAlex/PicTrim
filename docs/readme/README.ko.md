...
- 중첩 Form XObject와 inline image를 포함해 PDF 페이지 내용에 내장된 이미지를 처리합니다.

## PDF 동작

- “원본 형식 유지”는 PDF 컨테이너를 유지하고 페이지 이미지 객체만 교체합니다. 텍스트, 벡터, 링크, 폼, 페이지 크기와 배치는 유지됩니다.
- JPG / PNG / WebP는 PDF의 다른 내용을 버리고 고유 이미지마다 `<PDF 이름>/page-0001-image-0001.ext`로 한 번씩 출력합니다.
- JPEG, JPX, Flate, LZW, Gray/RGB/CMYK/Indexed/ICCBased, 8-bit 샘플과 일반 마스크를 지원합니다. PDF 미리보기는 아직 지원하지 않습니다.
- 암호가 필요한 PDF는 출력 없이 실패합니다. 서명 필드는 유지되지만 처리 후 원본 서명은 무효가 됩니다.
...
PicTrim은 [MIT License](../../LICENSE)로 배포됩니다.
