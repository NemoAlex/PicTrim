  ...
  <main class="shell">
    ...
    <section class="panel paths">
      ...
      <label>
        <span>输出目录</span>
        <div class="path-row">
          <input id="outputDir" readonly placeholder="选择保存处理结果的文件夹" />
    ...
    <section class="panel progress-panel">
      <div class="progress-head">
        <div>
          ...
          <p id="statusMessage">选择输入和输出目录后开始处理。</p>
        </div>
      ...
      <div class="stats">
        ...
        <div><span>复制</span><strong id="statCopied">0</strong></div>
    ...
    <section class="panel log-panel">
      <div class="log-head">
        <h2>失败列表</h2>
        <span id="currentFile">暂无正在处理的文件</span>
      </div>
      <div id="failures" class="failures">
        <div class="empty">没有失败文件。</div>
      </div>
    </section>
  </main>
`;
...
async function startBatch() {
  ...
  setRunning(true);
  failures.innerHTML = `<div class="empty">没有失败文件。</div>`;
  try {
